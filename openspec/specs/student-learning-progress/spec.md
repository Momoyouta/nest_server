# Student Learning Progress Specification

## Purpose
该规范定义了学生端视频播放进度与学习记录同步的业务逻辑与接口契约。
...
#### Scenario: 首次发送进度记录
- **WHEN** 学生第一次观看课时，并发送包含 `courseId`, `chapterId`, `lessonId` 及 `progress_percent=10` 和 `schoolId` 的请求
...
#### Scenario: 请求缺少必填参数
- **WHEN** 请求 Body 中缺少 `lessonId`、`schoolId` 或者 `progress_percent`
